...
var unixFs = new Unixfs('file', bufDataToHash);      console.log("02a",unixFs); 
                                                     console.log("02b",unixFs.data);          // <Buffer                   68 65 6c 6c 6f 20 77 6f 72 6c 64 73 0a>
...
var dagNode = dagPB.DAGNode.create(ufsDataToHash);   console.log("04a",dagNode,dagNode.Data,dagNode.Links);// <Buffer       08 02 12 0d 68 65 6c 6c 6f 20 77 6f 72 6c 64 73 0a 18 0d>
                                                     console.log("04b",dagNode.toJSON().data);// <Buffer       08 02 12 0d 68 65 6c 6c 6f 20 77 6f 72 6c 64 73 0a 18 0d>
...
var HashedData=sha256(dagDataToHash);                console.log("06a",HashedData);
var mHashedData = Buffer.concat([Buffer.from('1220', 'hex'),Buffer.from(HashedData, 'hex')]);console.log("06",mHashedData); // <Buffer 12 20 9f 66 8b 20 cf d2 4c db f9 e1 98 0f a4 86 7d 08 c6 7d 2c af 84 99 e6 df 81 b9 bf 0b 1c 97 28 7d>
...
var b58HashedData=multihash.toB58String(mHashedData);    console.log("07",b58HashedData);        // QmZ4tDuvesekSs4qM5ZBKpXiZGun7S2CYtEZRB3DYXkjGx
...
var HashDataV1 = Buffer.from(sha256(bufDataToHash),'hex');     console.log("11", HashDataV1);    // <Buffer         37 3e d7 8e 7d f1 d9 19 58 c2 60 bf 90 48 6b 01 73 39 8b 61 0b de 29 1b b5 76 44 66 2a 87 af 2a>
var bufPrefV1 = Buffer.from('01551220', 'hex');                    console.log("12",bufPrefV1);       // <Buffer 01 55 12 20> // 01 = cid version 1  // 0x55 = "U" ipld	raw binary // 12=sha2-256  // 20 256 bits
var bufExHashDataV1 = Buffer.concat([bufPrefV1,HashDataV1]);  console.log("13",bufExHashDataV1); // <Buffer 01 55 12 20 37 3e d7 8e 7d f1 d9 19 58 c2 60 bf 90 48 6b 01 73 39 8b 61 0b de 29 1b b5 76 44 66 2a 87 af 2a>
var b58HashedDataV1="z"+multihash.toB58String(bufExHashDataV1);console.log("14",b58HashedDataV1); // zb2rhaMwCEJiQ7FbitJJhtQMybzLetLwWJeHSSWzj5vJgguR3 //base58btc	z	base58 bitcoin  https://github.com/multiformats/multibase/blob/master/multibase.csv
